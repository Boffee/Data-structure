...
//Compresses the image this Quadtree represents.
    if (root == NULL) return;
    if (pruneCheck(tolerance, root, root->element)) {
        pruneClear(root);
        return;
    }
...
int Quadtree::tolerate(RGBAPixel const & curr, RGBAPixel const & next) const{
    return (int) (pow((curr.red - next.red),2) + pow((curr.green - next.green),2) + pow((curr.blue - next.blue),2));
}

bool Quadtree::pruneCheck(int tolerance, QuadtreeNode * curr, RGBAPixel const & compare) const
{
    if (curr->nwChild == NULL) {
...
int Quadtree::pruneSize (int tolerance, QuadtreeNode * curr) const{
    if (curr->nwChild == NULL) {
        return 1;
    }
    int i = 0;
    ...
    if(nw) i = i + pruneSize(tolerance, curr->nwChild);
    if(ne) i = i + pruneSize(tolerance, curr->neChild);
    if(sw) i = i + pruneSize(tolerance, curr->swChild);
    if(se) i = i + pruneSize(tolerance, curr->seChild);
...
//Calculates and returns the minimum tolerance necessary to guarantee that upon pruning the tree, no more than numLeaves leaves remain in the Quadtree.
    ...
    int tol = idealPrune(numLeaves, 0, 50 ,100);
    return tol;
}

int Quadtree::idealPrune (int numLeaves, int lo, int mid, int hi) const{
    int small = pruneSize(hi);
    if (pow((hi-lo),2) == 1) {
        if(small >= numLeaves) return hi;
        else return lo;
    }
    if (hi == lo) {
        if (small >= numLeaves) return hi;
    }
    int medium = pruneSize(mid);    
    if (numLeaves < small){
        return idealPrune(numLeaves, hi, (3*hi)/2 , 2*hi);
    }

    if (numLeaves >= medium){
        std::cout << numLeaves << " > " << medium << std::endl;
        return idealPrune(numLeaves, lo, (lo+mid)/2, mid);
    }
    else{
        std::cout << numLeaves << " < " << medium << std::endl;
        return idealPrune(numLeaves, mid, (mid+hi)/2, hi);
    }
}
