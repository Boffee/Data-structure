...
class KDTree
{
    ...
    private:
		...
		int sqdistance(const Point<Dim> & first, const Point<Dim> & second) const;

		// ctor helper function
		void buildTree(vector< Point<Dim> > & newPoints, int start, int end, int curDim);
    
        //return median point
		Point<Dim> findMedian(vector< Point<Dim> > & newPoints,int start, int end, int curDim);
    
        //sort helper function
		void sort(vector< Point<Dim> > & newPoints,int start, int end, int mid, int curDim);
        /**
         * @todo Add your helper functions here.
         */
		
		Point<Dim> findNearestNeighbor(const Point<Dim> & target, const Point<Dim> & currentBest, int start, int end, int curDim) const;
		
		Point<Dim> split(const Point<Dim> & target, const Point<Dim> & currentBest, const Point<Dim> & potential,int start, int end, int curDim, bool rl) const;
};
