...
void HuffmanTree::buildTree( const vector<Frequency> & frequencies ) {
    ...
    for (int i = 0; i < frequencies.size(); i++){
        TreeNode * tl = new TreeNode(frequencies.at(i));
        tl->left = NULL;
        tl->right = NULL;
        singleQueue.push(tl);
    }
    ...
    while (singleQueue.size() + mergeQueue.size() > 1){
        ...
        ti = new TreeNode(right->freq.getFrequency()+left->freq.getFrequency());
        ti->left = left;
        ti->right = right;
        mergeQueue.push(ti);
        cout << "end" << endl;
...
void HuffmanTree::decode( stringstream & ss, BinaryFileReader & bfile ) {
	...
	while( bfile.hasBits() ) {
        ...
        if( !bfile.hasBits()) ss << current->freq.getCharacter();
		/**
		 * @todo Your code here!
		 *
...
HuffmanTree::TreeNode * HuffmanTree::readTree( BinaryFileReader & bfile ) {
    ...
    if (bfile.hasBytes()) {
        if(bfile.getNextBit() == 1){
            Frequency t(bfile.getNextByte(),0);
            TreeNode * tl = new TreeNode(t);
            tl->left = NULL;
            tl->right = NULL;
            return tl;
        }
        else {
            TreeNode * ti = new TreeNode(0);
            ti->left = readTree(bfile);
            ti->right = readTree(bfile);
            return ti;
        }
    }
