...
RGBAPixel gradientColorPicker::operator()(int x, int y)
{
	...
	d = abs(xc-x) + abs(yc-y);
	if (d >= rad) d = rad;
	color.red = startColor.red + floor(d*(endColor.red)/rad) - floor(d*startColor.red/rad);
	color.green = startColor.green +floor(d*(endColor.green)/rad) - floor(d*startColor.green/rad);
	color.blue = startColor.blue + floor(d*(endColor.blue)/rad) - floor(d*startColor.blue/rad);
