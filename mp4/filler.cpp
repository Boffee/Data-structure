...
animation filler::dfs::fillSolid( PNG & img, int x, int y, 
     ...
     */
    colorPicker * solid = new solidColorPicker(fillColor);
    animation anim = filler::fill<Stack>(img,x,y,*solid,tolerance,frameFreq);
    delete solid;
    return anim;
}
...
animation filler::dfs::fillGrid( PNG & img, int x, int y, 
     ...
     *  correct call to fill with the correct colorPicker parameter.
     */
    colorPicker * grid = new gridColorPicker(gridColor,gridSpacing);
    animation anim =  filler::fill<Stack>(img,x,y,*grid,tolerance,frameFreq);
    delete grid;
    return anim;
}
...
animation filler::dfs::fillGradient( PNG & img, int x, int y, 
     ...
     *  correct call to fill with the correct colorPicker parameter.
     */
    colorPicker * grad = new gradientColorPicker(fadeColor1, fadeColor2, radius, x, y);
    animation anim =  filler::fill<Stack>(img,x,y,*grad,tolerance,frameFreq);
    delete grad;
    return anim;
}
...
animation filler::bfs::fillSolid( PNG & img, int x, int y, 
     ...
     */
    colorPicker * solid = new solidColorPicker(fillColor);
    animation anim =  filler::fill<Queue>(img,x,y,*solid,tolerance,frameFreq);
    delete solid;
    return anim;
}
...
animation filler::bfs::fillGrid( PNG & img, int x, int y, 
     ...
     */
    colorPicker * grid = new gridColorPicker(gridColor,gridSpacing);
    animation anim =  filler::fill<Queue>(img,x,y,*grid,tolerance,frameFreq);
    delete grid;
    return anim;
}
...
animation filler::bfs::fillGradient( PNG & img, int x, int y, 
     ...
     */
    colorPicker * grad = new gradientColorPicker(fadeColor1, fadeColor2, radius, x, y);
    animation anim =  filler::fill<Queue>(img,x,y,*grad,tolerance,frameFreq);
    delete grad;
    return anim;
}
...
animation filler::fill( PNG & img, int x, int y,
    ...
    OrderingStructure< int > * xl = new OrderingStructure<int>();
    OrderingStructure< int > * yl = new OrderingStructure<int>();
    ...
    while(!(xl->isEmpty())){
        ...
        if (arr[xt][yt] == false){
            arr[xt][yt] = true;
            int diff = pow((orig.red - img(xt,yt)->red),2) + pow((orig.blue - img(xt,yt)->blue),2) + pow((orig.green - img(xt,yt)->green),2);
            if (diff <= tolerance){
                //x right
                if (xt < img.width()-1){
                    xl->add(xt+1);
                ...
                if (yt > 0){
                    xl->add(xt);
                    yl->add(yt-1); //neg is up
                }
                RGBAPixel npix = fillColor(xt,yt);
                img(xt,yt)->red = npix.red;
                img(xt,yt)->blue = npix.blue;
                img(xt,yt)->green = npix.green;
                k++;
                if (k == frameFreq){
                    k = 0;
                    count++;
